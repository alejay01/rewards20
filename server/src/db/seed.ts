...
async function main() {
  ...
  try {
    ...
    const roleAdmin = await db.insert(roles).values({ name: "Administrator", description: "Full access control" });
    const roleManager = await db.insert(roles).values({ name: "Manager", description: "Can manage store staff, redemptions, and view logs" });
    const roleTeam = await db.insert(roles).values({ name: "Team Member", description: "Can scan, view status, and award standard points/visits" });
    ...
    const getPermId = (key: string) => allPerms.find(p => p.key === key)!.id;
    ...
    for (const key of teamPerms) {
      await db.insert(rolePermissions).values({ roleId: teamRoleId, permissionId: getPermId(key) });
    }

    // 5. Seed Staff Users (Demo credentials)
    console.log("Seeding staff users...");
    const demoPasswordHash = bcrypt.hashSync("BoudinBoss2026!", 10);
    const demoPinHash = bcrypt.hashSync("1234", 10); // Standard override PIN for managers
    ...
    for (const t of tierData) {
      await db.insert(tiers).values(t);
    }
