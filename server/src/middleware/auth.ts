...
import { db } from "../db";
import { staffUsers, roles, rolePermissions, permissions, settings, authorizedDevices } from "../db/schema";
...
export const validateSecurityAccess = async (req: Request, res: Response, next: NextFunction) => {
  ...
  else if (ua.includes("iPad")) operatingSystem = "iOS (iPad)";
  ...
  if (ua.includes("iPad") || (ua.includes("Macintosh") && "ontouchend" in global)) deviceType = "Tablet";
  else if (ua.includes("Mobile") || ua.includes("Android") || ua.includes("iPhone")) deviceType = "Phone";

  try {
    // Bootstrap: if zero approved devices, auto-approve this first one
    const countApproved = await db.select().from(authorizedDevices).where(eq(authorizedDevices.status, 'approved'));
    ...
    // Lookup device
    const devices = await db.select().from(authorizedDevices).where(eq(authorizedDevices.deviceFingerprint, fingerprint));
...
function decimalToMaybeString(val: any): string | null {
